...
A data analysis and lead-prioritization tool built for a bank's telemarketing campaign, using a dataset of 41,187 (`bank-additional-full.csv`) phone-marketing contacts to identify which clients are most likely to subscribe to a term deposit, so outreach can be prioritized toward the leads most worth calling.
